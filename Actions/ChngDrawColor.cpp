...
void ChngDrawColor::Execute()
{
    ...
    for (int i = 0; i < FigCount; i++)
    {
        ...
        {
            // Update the fill color (will automatically set isFilled to true)
            FigList[i]->ChngDrawClr(FillClr);
            updatedCount++;
        }
    }
    ...
    if (updatedCount > 0)
        pOut->PrintMessage(to_string(updatedCount) + " Draw color of the selected figure is updated");
    else
